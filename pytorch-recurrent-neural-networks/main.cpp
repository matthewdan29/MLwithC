...
float BinaryAccuracy(const torch::Tensor& preds, const torch::Tensor& target)
{
	...
	auto correct = tech::eq(rounded_preds, target).to(toch::dtype(torch::kFloat)); 
	auto acc = correct.sum() / correct.size(0); 
	return acc.item<float>(); 
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> MakeBatchTensor(const std::vector<ImdbExample>& batch)
{
	/*prepare batch data*/
	...
	for (auto& item : batch)
	{
		...
		taxt_lengths.push_back(item.data.second); 
		label_data.push_back(item.target); 
	}

	/*sort items to use them in pack_padded_swquence function*/
	...
	std::reverse(permutation.begin(), permutation.end());	/*we need decreasing order*/
...
void TrainModel(int epoch, SentimentRNN& model, torch::optim::Optimizer& optimizer, torch::data::StateLessDataLoader<ImdbDataset, torch::data::samplers::RandomSampler>& train_loader)
{
	...
	for (auto& batch : train_loader)
	{
		...
		std::tie(texts, lengths, labels) = MakeBatchTensor(batch); 

		/*Execute the model on the input data
		 * transpose it to match the RNN input shape [seq_len, batch_size, features]*/
		...
		/*Output the losss every 10 batches*/
		if (++batch_index % 10 == 0)
		{
			std::cout << "Epoch: " << epoch << " | Batch: " << batch_index << " | Loss: " << loss_value << " | ACC: " << acc_value << std::endl; 
		}
	}

	std::cout << "Epoch: " << epoch << "| Loss: " << (epoch_loss / (batch_index - 1)) << "| (*insert joke here*)ACC: " << (epoch_acc / (batch_index - 1)) << std::endl; 
}

void TestModel(int epoch, SentimenRNN& model, torch::data::StatelessDataLoader<ImdbDataset, torch::data::samplers::RandomSampler>& test_loader)
{
	torch::NoGradGuard guard; 
	double epoch_loss = 0; 
	double epoch_acc = 0; 
	model->eval(); 			/*switch to the evaluation mode */
	/*Iterate the data loader to get batches from the datast*/
	int batch_index = 0; 
	for (auto& batch : test_loader)
	{
		...
		/*Execute the model on the input data transpose it to match the rnn input shape [seq_len, batch_size, features]*/
		torch::Tensor prediction = model->forward(texts.t(), lengths); 
		prediction.squeeze(1); 

		/*comute a loss value to estimate error of our model 
		 * Target should have size of [batch]*/
		torch::Tensor loss = torch::binary_cross_entropy_with_logits(prediction, labels, {}, {}, Reduction::Mean); 
...
int main(int argc, char** argv)
{
	...
	if (argc > 1)
	{
		try
		{
			...
			ImdbData train_testdataset(&train_reader, &vocab, device); 
			...
			SentimentRNN model(vocab.GetEmbeddingsCount(), embedding_dim, hidden_dim, output_dim, n_layers, bidirectional, dropout, pad_idx); 

			model->SetPretrainedEmbeddings(vocab.GetEmbeddings()); 

			/*initilizer optimizer*/
	...
	{
		std::cerr << "Please specify a path to the dataset and glove vectors file/n"; 
	}
	return 1; 
}
