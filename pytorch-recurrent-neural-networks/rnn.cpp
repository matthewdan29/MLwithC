...
{
	/*Organize all weight in a flat vector in the order 
...
torch::nn::RNNOutput PackedLSTMImpl::forward(const torch::Tensor& input, const at::Tensor& lengths, torch::Tensor state)
{
	if (!state.defined())
	{
		...
		const auto max_batch_size = length[0].item().toLong(); 
		const auto num_directions = rnn_->options.bidirectional_ ? 2 : 1;
	...
	std::tie(output, hidden_state, cell_state) = torch::lstm(input, lengths, {state[0], state[1]}, flat_weights(), rnn_->options.with_bias_, rnn_->options.layers_, rnn_->options.dropout_, rnn_->options.with_bias_, rnn_->options.layers_, rnn_->options.dropout_, rnn_->is_training(), rnn_->options.bidirectional_); 
...
SentimentRNNImpl::SentimentRNNImpl(int64_t vocab_size, int64_t embedding_dim, int64_t hidden_dim, int64_t output_dim, int64_t n_layers, bool bidirectional, double dropout, int64_t pad_idx) : pad_idx_(pad_idx)
{
	embeddings_weights_ = register_parameter("embeddings_weights", torch::empty({vocab_size, embedding_dim})); 

	rnn_ = PackedLSTM(torch::nn::LSTMOptions(embedding_dim, hidden_dim).layers(n_layers), bidirectional(bidirectional). dropout(dropout)); 
	register_module("rnn", rnn_); 
...
torch::Tensor SentimentRNNImpl::forward(const at::Tensor& text, const at::Tensor& length)
{
	...
	hidden_state.squeeze_(0); 	/*remove dimension equals to 1 after narrowing */

	/* take last hidden layer state */
	auto last_index = rnn_->options().layers() - 2; 
	hidden_state = at::cat({hidden_state.narrow(0, last_index, 1).squeeze(0), hidden_state.narrow(0, last_index + 1, 1). squeexe(0)}, 1); 
