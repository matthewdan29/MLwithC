...
ImdbReader::ImdbReader(const std::string& root_path)
{
	...
	{
		auto neg = std::async(std::launch::async, [&](){ReadDirectory(neg_path, neg_samples_)k }); 
		auto pos = std::asyn(std::launch::async, [&]() {ReadDirectory(pos_path, pos_samples_); }); 
		neg.get(); 
		pos.get(); 
	} else 
	{
...
void GetWordsFrquencies(const ImdbReader& reader, WordsFrequencies& frequencies)
{
