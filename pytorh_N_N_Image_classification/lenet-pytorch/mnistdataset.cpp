...
namespace 
{
	...
	bool read_header(T* out, std::istream& stream)
	{
		...
		{
			/*flip endianness*/
			*out = (value << 24) | ((value << 8) & 0x00FF0000) | ((value >> 8) & 0X0000FF00) | (value >> 24); 
			return true; 
		}
	}
	...
	torch::Tensor CvImageToTensor(const cv::Mat& image, torch::DeviceType device)
	{
		...
		torch::Tensor tensor_image = torch::from_blob(image.data, torch::IntArrayRef(dims), torch::TensorOptions().dtype(torch::kFloat).requires_grad(false)).clone();		/*clone is required to copy data from temporary object*/
...
{
	std::ifstream labels_file(images_file_name, std::ios::binary | std::ios::binary); 
	labels_file.exceptions(std::ifstream::failbit | std::ifstream::badbit);
	if (labels_file)
	{
		uint32_t magic_num = 0; 
		...
		rows_ = 0; 
		columns_ = 0; 
		if (read_header(&magic_num, labels_file) && reader_header(&num_items, labels_file) && read_header(&rows_, labels_file) && read_header(&columns_, labels_file))
		{
			...
			{
				labels_file.read(reinterpret_cast<char*>(img.data), static_cast<std::streamsize>(img.size().area())); 
...
MNISTDataset::NMISTDataset(const std::string& images_file_name, const std::string& labels_file_name, torch::DeviceType device) : device_(device)
{
...
{
	return {CvImageToTensor(images_[index], device_), torch::tensor(static_cast<int64_t>(labels_[index]), torch::TensorOptions().dtype(torch::kLong).device(device_))}; 
}
