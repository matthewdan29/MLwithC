...
const std::vector<std::string> data_names{"dataset0.csv", "dataset1.csv", "dataset2.csv", "dataset3.csv", "dataset4.csv"}; 
...
void PlotClasses(const Classes& classes, const std::string& name, const std::string& file_name)
{
	...
	for (auto& cls : classes)
	{
		...
		plt.AddDrawing(draw_state, plotcpp::Points(cls.second.first.begin(), cls.second.first.end(), cls.second.second.begin(), std::to_string(cls.first) + " cls", params.str())); 
	}
...
void KNNClassification(const ClassificationDataset& train, const ClassificationDataset& test, unsigned int num_classes, const std::string& name)
{
	...
	PlotClasses(classes, "kNN " + std::to_string(accuracy), name + "-knn-shrkml.png"); 
}

void LRClassification(const ClassificationDataset& train, const ClassificationDataset& test, unsigned int num_classes, const std::string& name)
{
	...
	{
		std::vector<OneVersusOneClassifier<RealVector>::binary_classifier_type*> ovo_classifiers; 
	...
	PlotClasses(classes, "Logistic Regression " + std::to_string(accuracy), name + "-logreg-sharkml.png"); 
}

void SVMClassification(const ClassificationDataset& train, const ClassificationDataset& test, unsigned int num_classes, const std::string& name)
{
	double c = 10.0; 
	double gamma = 0.5; 
	...
	for (std::size_t n = 0, cls1 = 1; cls1 < num_classes; cls1++)
	{
		std::vector<OneVersusOneClassifier<RealVector>::binary_classifier_type*> ovo_classifier; 
		for (std::size_t cls2 = 0; cls2 < cls1; cls2++, n++)
		{
			/*get the binary subproblem*/
			ClassificationDataset binary_cls_data = binarySubProblem(train, cls2, cls1); 
			...
			ovo_classifier.push_back(&svm[n]); 
		}
		ovo.addClass(ovo_classifier); 
	}
	...
	for (std::size_t i = 0; i != test.numberOfElements(); i++)
	{
		...
		auto element = test.inputs().element(i); 
		classes[cluster_idx].first.push_back(element(0)); 
		classes[cluster_idx].first.push_back(element(1)); 
	}

	PlotClasses(classes, "SVM " + std::to_string(accuracy), name + "-svm-sharkml.png"); 
}
	...
	if (argc > 1)
	{
		...
		for (auto& dataset : data_names)
		{
			...
			if (fs::exists(dataset_name))
			{
				...
				data = selectInputFeatures(data, std::vector<int>{1, 2}); 		/* exclude index column*/
				...
				std::cout << dataset << "\n" << "Num samples: " << num_samples << " num features: " << num_features << " numcluster: " << num_classes << std::endl; 
