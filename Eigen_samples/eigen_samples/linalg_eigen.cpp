...
{
	{
		...
		a(0, 0) = 3; 
		std::cout << "Matrix with change element[0][0]:\n" << a << std::endl;
	...
	/*patial access*/
	{
		...
		Eigen::Matrix2f b = m.block(1, 1, 2, 2);	/*copy the middle part of matrix*/
		std::cout << "Middle of 4X4 matrix :\n" << b << std::endl; 

		m.block(1, 1, 2, 2) *= 0; 	/*Change values in orginal matrix*/
	...
	/*broadcasting*/
	{
		...
		v << 100, 100; 
		mat.colwise() += v; 
		std::cout << "Sum bradcasted over columns :\n" << mat << std::endl; 
	}
