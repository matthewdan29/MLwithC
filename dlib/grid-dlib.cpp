...
int main(int argc, char** argv)
{
	...
	auto mm = std::minmax_element(samples.begin(), samples.end(), [](const auto& a, const auto& b) {return a(0, 0) < b(0, 0); }); 
	...
	auto result = find_min_global(CrossValidationScore, {0.01, 1e-8, 5}, {0.1, 1, 15}, max_function_calls(50)); 
	...
	std::transform(new_samples.begin(), new_samples.end(), predictions.begin(), descision_func); 
	...
	plt.Draw2D(plotcpp::Points(x_coords.begin(), x_coords.end(), raw_labels.begin(), "orig", "lc rbg 'black' pt 7"), plotcpp::Lines(x_pred_coords.begin(), x_pred_coords.end(), predictions.begin(), "pred", "lc rgb 'red' lw 2")); 
