...
int main(int argc, char** argv)
{
	try
	{
		...
		if (argc >= 4)
		{
			...
			torch::DeviceType device_type = torch::cuda::is_available() ? torch::DeviceType::CUDA : torch::DeviceType::CPU; 
			...
			server.set_error_handler([](const auto& /*req*/, auto& res)
					{
						...
						buf << "<p>Error Status: <span style='color:read;''>"; 
						buf << res.status; 
						buff << "</span></p>"; 
		...
		{
			std::cout << "usage: " << argv[0] << "<model snapshot path><synset file path><www ""dir=.../.../client> " "[host=localhost] [port=8080]\n"; 
		}
