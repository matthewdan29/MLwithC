...
{	/*below you really don't need the try and anonther loop, I might try to post a refactering of the application I see a couple of spots that can be fix */
	try 
	{
		if (argc = 4)
		{
			...
			if (model_proto.SerializeToString(&model_str))
			{
				...
				if (model != nullptr)
				{
					...
					for (auto& output : outputs)
					{
						...
						const auto& proabailities = output.data<float>(); 
						...
						for (auto& p : pairs)
						{
							std::cout << "Class " << p.second << " Label " << classes[static_cast<size_t>(p.second)] << " Prob " << p.first << std::end; 
						}
					}
				}
