...
struct ReviewsHandler : public rapidjson::BaseReaderHandler<rapidjson::UTF8<>, ReviewsHandler> { ReviewsHandler(Papers* papers) : papers_(papers){}

	bool Unit(unsigned u)
	{
		bool res{true}; 
		try
		{
			...
			{
				paper_.id = u;		/*Change "paper_.id" to "papers_.id"*/ 
	...
	bool String(const char* str, rapidjson::SizeType length, bool)
	{
		...
		try
		{
			...
			{
				review_.orentation = std::string(str, length); 
