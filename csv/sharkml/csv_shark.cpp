...
#include <shark/Model/Normalizer.h>
...
int main(int argc, char** argv)
{
	...
	if (argc > 1)
	{
		if (fs::exists(argv[1]))
		{
			/*we need to preprocess dataset because SharkML fails to load csv with string values*/
			...
			data_string = std::regex_replace(data_string, std::regex("Iris-verginca"), "3"); 
