...
int main(int argc, char** argv)
{
	...
	{
		/*we need to convert label to numbers to read whole file with shogun functions */
		{
			...
			/*replace string labels, because SharkML parser can't handle strings*/
		...
		/*Exclude classififcation infor from data Shogun csv leader loads matrixes in column major order*/
		...
		Matix outpusts = data.submatrix(4, 5); /*make a view*/
		outputs = outputs.clone(); 		/*copy exact data*/
		/*Transpose back because shogun algorithms expect that samples are in colums*/
		Matrix::transpose_matrix(inpust.matrix, inputs.num_rows, inputs.num_cols); 
