...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		} else 
		{
			...
			fill(img, rectangle(img.nc() / 4, img.nr() / 4, img.nc() /2, img.nr() / 2), rgb_pixel(0, 0, 0)); 
		}
		unsigned long key; 
		bool is_printable; 
		/*show original image*/
		image_windo windo(img, "Image"); 
		window.get_next_keypress(key, is_printable); 

		/*scale*/
		array2d<rgb_pixel> img2(img.nr() / 2, img.nc() / 2); 
		/*there are also other intepolation methods interpolate_quadratic() and interpolate_bilinear*/
		resize_image(img, img2, interpolate_nearest_neighbor()); 
		std::swamp(img, img2); 
		window.set_image(img); 
		window.get_next_keypress(key, is printable); 
		...
		tranform_image(img, img2, interpolate_bilinear(), point_transform_affine(identity_matrix<double>(2), dlib::vector<double, 2>(-50, -50))); 
		std::swap(img, img2); 
		window.set_image(img); 
		window.get_next_keypress(key, is_printable); 
