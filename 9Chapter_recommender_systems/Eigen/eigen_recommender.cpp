#include <omp.h>
/* #define EIGEN_DONT_PARALLELIZE (*FOR CLOUD NOVA*)*/
#include <Eigen/Core>
#include <Eigen/Dense>
...
/* matrices in ColMajor order (meaning in the array that the matrcis are in are ordered in colomn order)*/
...
int main(int argc, char** argv)
{
	if (argc == 2)
	{
		...
		#pragma omp for private(diff, xtcux, a, b, update_x)
			...
			double elapsed_seconds = std::chrono::duration_cast<std::chrono::duration<double>>(finish_time - start_time).count(); 
			std::cout << "Initeration " << k << " weighted mse " << w_mse << " time " << elapsed_seconds <<std::endl; 
			}
